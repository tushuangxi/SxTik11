package com.tushuangxi.smart.tv.lding.widget.dragcircleprogress;
...
public class DragCircleProgress extends View {
    ...
    String TAG = "TAG: "+ DragCircleProgress.class.getSimpleName()+"....";
    /**
     * 画外圆的颜色
     */
...
//                onStopTrackingTouch();
                if (mCurrentValue > 1 * mTotalValue) {
                    mCurrentValue = 1 * mTotalValue;
                    mTouchAngle = 0;
                    circleCount = 1;
                    Toast.makeText(getContext(),"设置延时关灯最大时间为1小时",Toast.LENGTH_SHORT).show();
                }
                if (circleCount < 0) {
                    circleCount = 0;
                    mTouchAngle = 0;
