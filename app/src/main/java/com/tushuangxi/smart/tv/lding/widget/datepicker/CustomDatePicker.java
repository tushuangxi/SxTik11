package com.tushuangxi.smart.tv.lding.widget.datepicker;
 ...
 *         mDatePicker.setCancelable(false); // 不允许点击屏幕或物理返回键关闭
 *         mDatePicker.setCanShowPreciseTime(false); // 不显示时和分
 *         mDatePicker.setScrollLoop(false); // 不允许循环滚动
 *         mDatePicker.setCanShowAnim(false);// 不允许滚动动画
 *     }
 *     //年月日时分
 ...
 *         mTimerPicker.setScrollLoop(true); // 允许循环滚动
...
//            lp.height = WindowManager.LayoutParams.WRAP_CONTENT;
    ...
    public void show(String dateStr) {
        ...
        // 弹窗时，考虑用户体验，不展示滚动动画
        //默认为当前日期
        String nowDateStr = DateUtil.myNowSystemtime();
        if (setSelectedTime(nowDateStr, false)) {
            mPickerDialog.show();
        }
    }
