...
public class AppUtils2 {
    ...
    public static String getAppSign(Context context, String packageName) {
        try {
            ...
            throw new RuntimeException(AppUtils2.class.getName() + "the " + packageName + "'s application not found");
        }
    }
...
//  }
     ...
     * @return
     */
    public static boolean checkAppInstalled(Context context, String pkg, String cls){
        PackageManager pm = context.getPackageManager();
        Intent intent = new Intent();
        ComponentName cmp = new ComponentName(pkg, cls);
        intent.setComponent(cmp);
        List<ResolveInfo> rf = pm.queryIntentActivities(intent,PackageManager.MATCH_ALL);
        return rf.size() > 0;
    }
}
